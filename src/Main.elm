...
movesForPlayer coord state =
    case getTileAt coord state.tiles of
        Just tile ->
            case tile of
                Tile tileVal ->
                    tileVal
...
addIndexIfMatch target index value =
...
getRedStart board =
    indexToXy (List.sum (List.indexedMap (addIndexIfMatch RedStart) board))


getBlueStart : List Tile -> Coord
getBlueStart board =
    indexToXy (List.sum (List.indexedMap (addIndexIfMatch BlueStart) board))
...
update msg state =
    case msg of
        MoveTo x y ->
            let
                updatedVisited =
                    List.append [ xyToIndex ( x, y ) ] state.visited
            in
            let
                updatedState =
                    { state | badMoveReason = Nothing, tiles = List.indexedMap (collapseAt state.collapseNext) state.tiles, visited = updatedVisited }
            in
            let
                postMoveState =
            ...
            case state.history of
                History priorHistory ->
                    ( { postMoveState | history = History (List.append [ state ] priorHistory) }, Cmd.none )
        ...
        Undo ->
            case state.history of
                History previous ->
                    safeUndoHead previous

        InvalidMoveAttempted badBecause ->
            ( { state | badMoveReason = Just badBecause }, Cmd.none )

        ShuffledBoard newTiles ->
            ( { initModel | tiles = newTiles, blueAt = getBlueStart newTiles, redAt = getRedStart newTiles, collapseNext = getBlueStart newTiles }, Cmd.none )
...
checkTileIsAccesible ( toX, toY ) state =
    let
        ( fromX, fromY ) =
            getFromPos state
    in
    let
        ( oppX, oppY ) =
            getOpponentPos state
    in
    if (oppX == toX) && (oppY == toY) && (state.movesLeft == 1) then
        InvalidMoveAttempted "Can't finish on opponent's tile"

    else if List.member (xyToIndex ( toX, toY )) state.visited then
        InvalidMoveAttempted "Can't re-visit a tile during a turn"

    else if abs (fromX - toX) + abs (fromY - toY) == 1 then
        MoveTo toX toY

    else if (fromX == 3) && (toX == 0) && (fromY == toY) then
        MoveTo toX toY

    else if (fromY == 3) && (toY == 0) && (fromX == toX) then
        MoveTo toX toY

    else if (fromX == 0) && (toX == 3) && (fromY == toY) then
        MoveTo toX toY

    else if (fromY == 0) && (toY == 3) && (fromX == toX) then
        MoveTo toX toY

    else
        InvalidMoveAttempted "Can't move to non-adjacent tile."


checkMoveValid : Coord -> Model -> Msg
checkMoveValid coord state =
    case getTileAt coord state.tiles of
        Just tile ->
            case tile of
                Collapsed ->
                    InvalidMoveAttempted "Can't move to collapsed tile"

                _ ->
                    checkTileIsAccesible coord state

        Nothing ->
            InvalidMoveAttempted "Somehow clicked a tile out of range"


moveMessageFromIndex : Int -> Model -> Msg
moveMessageFromIndex index state =
    let
        ( x, y ) =
            indexToXy index
    in
...
tileAttrs index blueIndex redIndex state =
    [ if index == blueIndex && index == redIndex then
        style "background" "linear-gradient(to right, blue 0%, blue 50%, red 50%, red 100%)"

      else if index == blueIndex then
        style "background-color" "blue"

      else if index == redIndex then
...
renderTile state index tile =
    let
        blueAtIndex =
            xyToIndex state.blueAt
    in
    let
        redAtIndex =
            xyToIndex state.redAt
    in
    case tile of
        Collapsed ->
            div (collapsedAttrs index state) [ text "x" ]

        Tile x ->
            div (tileAttrs index blueAtIndex redAtIndex state) [ text (String.fromInt x) ]

        RedStart ->
            div (tileAttrs index blueAtIndex redAtIndex state) [ text "Red Start" ]

        BlueStart ->
            div (tileAttrs index blueAtIndex redAtIndex state) [ text "Blue Start" ]
...
gameExplain =
    [ text "Collapsi was invented by 'Riffle Shuffle and Roll' on youtube, you can watch him explain the rules "
...
view state =
    ...
    , body =
        [ div [ globalStyle ]
            ...
            , case state.history of
                History [] ->
                    button [] [ text "Undo Last Move" ]

                _ ->
                    button [ onClick Undo ] [ text "Undo Last Move" ]
...
messageFromNewBoard newBoard =
    ShuffledBoard newBoard


shuffleBoard : List Tile -> Cmd Msg
shuffleBoard board =
    Random.generate messageFromNewBoard (shuffle board)
...
init _ =
    ( initModel, shuffleBoard initialBoard )
